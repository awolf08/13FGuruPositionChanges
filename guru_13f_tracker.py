...
def write_html_chart(path: Path, groups: list[dict[str, object]]) -> None:
    ...
    lines = [
        ...
        ".report-header { display: flex; justify-content: space-between; gap: 20px; align-items: start; margin-bottom: 18px; }",
        ".report-title { min-width: 0; }",
        ".creator-badge { display: grid; grid-template-columns: 54px max-content; gap: 10px; align-items: center; padding: 8px 10px; border: 1px solid #dde2ea; border-radius: 8px; background: #fff; box-shadow: 0 6px 18px rgba(31, 41, 55, 0.06); }",
        ".creator-avatar { width: 54px; height: 54px; border-radius: 50%; object-fit: cover; border: 2px solid #d7e8ff; }",
        ".creator-label { margin: 0 0 2px; color: #6b7280; font-size: 11px; text-transform: uppercase; letter-spacing: 0.04em; }",
        ".creator-name { margin: 0; color: #111827; font-size: 14px; font-weight: 800; white-space: nowrap; }",
        ...
        "@media (max-width: 640px) { body { padding: 14px; } .report-header { display: block; } .creator-badge { width: fit-content; margin: 12px 0 0; } .piebox { grid-template-columns: 1fr; } .pie { width: min(100%, 260px); } .movers { grid-template-columns: 1fr; } .summary-head { display: block; } .top-row { grid-template-columns: 34px minmax(0, 1fr); } .top-pct, .top-holders { text-align: left; grid-column: 2; } }",
        "</style>",
        "</head>",
        "<body><main>",
        ...
        "</div>",
        '<div class="creator-badge" title="Built by Weicheng">',
        '<img class="creator-avatar" src="creator-avatar.jpg" alt="Weicheng creator avatar">',
        "<div>",
        '<p class="creator-label">Built by</p>',
        '<p class="creator-name">Weicheng</p>',
        "</div>",
        "</div>",
        "</div>",
        '<div class="legend">',
