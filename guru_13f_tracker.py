...
def write_html_chart(path: Path, groups: list[dict[str, object]]) -> None:
    ...
    lines = [
        ...
        "<title>Guru Position Changes</title>",
        "<style>",
        ...
        ".creator-badge { display: block; padding: 6px; border: 1px solid #dde2ea; border-radius: 50%; background: #fff; box-shadow: 0 6px 18px rgba(31, 41, 55, 0.06); line-height: 0; }",
        ...
        "<h1>Guru Position Changes</h1>",
        f'<p class="note">Generated {html.escape(datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC"))}. Pie slices are total portfolio weight. Positions below {min_portfolio_pct:.0f}% are grouped as Other.</p>',
        "</div>",
    ...
    if top_positions:
        ...
        lines.extend(
            [
                ...
                "<p>Each row sums the latest portfolio weight for the same issuer across all tracked investors.</p>",
                "</div>",
